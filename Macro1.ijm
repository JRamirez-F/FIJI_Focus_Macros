...
run("Threshold...");
waitForUser("Choose threshold \n Click Apply IN THE THRESHOLD CONSOLE ⚠️ \n Then press OK in this console");
run("Convert to Mask");

// === Morphological Cleaning ===
Cycles = getNumber("How many times to run Dilate/Erode?", 6);

setBatchMode(true);
setBatchMode("hide");

for (i = 0; i < Cycles; i++) run("Dilate");
run("Close-");
run("Fill Holes");
for (i = 0; i < Cycles; i++) run("Erode");
